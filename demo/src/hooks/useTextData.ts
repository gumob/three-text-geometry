...
export function useTextData() {
  const staticIndexRef = useRef(Math.floor(Math.random() * textList.length));

  const staticText = useCallback(() => {
    return textList[staticIndexRef.current];
  }, []);
  ...
  return { textList, staticText, randomText };
}
