...
class BMFontBinaryParser {
    static HEADER = [66, 77, 70];
    parse(buf) {
        if (buf.length < 6)
            throw new error_1.BMFontError('Invalid buffer length');
        const header = BMFontBinaryParser.HEADER.every((byte, i) => {
            return buf.readUInt8(i) === byte;
        });
        if (!header)
            throw new error_1.BMFontError('Missing BMF byte header');
        let i = 3;
        const vers = buf.readUInt8(i++);
        if (vers > 3)
            throw new error_1.BMFontError('Only supports bitmap font binary v3');
        ...
        try {
            ...
            throw new error_1.BMFontError(e.message);
        }
        return target;
    }
