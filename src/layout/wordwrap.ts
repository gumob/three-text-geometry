...
function lines(text: string, option: any = {}): WordMetrics[] {
  ...
  if (option.mode === WordWrapMode.Pre) return pre(opt.measure!, text, opt.start, opt.end!, opt.width!)
  else return greedy(opt.measure!, text, opt.start, opt.end!, opt.width!, opt.mode!)
}
